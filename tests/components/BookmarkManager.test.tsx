...
describe('BookmarkManager', () => {
  ...
  const mockFolders = [
    { id: 1, fid: 1, mid: 1, title: '默认收藏夹', description: '', count: 2, type: 0 },
  ];

  test('should render video list', () => {
    render(<BookmarkManager videos={mockVideos} folders={mockFolders} onAction={jest.fn()} />);
  ...
  test('should select videos for batch operation', () => {
    ...
    render(<BookmarkManager videos={mockVideos} folders={mockFolders} onAction={onAction} />);
    ...
    expect(onAction).toHaveBeenCalledWith('delete', expect.arrayContaining(['BV1']));
  });
